...
def add_obj_map(top_array, obj, depth_frame, rgb_ints):
    ...
    if 0 <= center[1] <= 400 and h_top <= 585:
        ...
        if obj[0] < 10:
            cv2.putText(top_array, str(int(obj[0])), (center[0] - 3,
                        center[1] + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, 0, 1)
        else:
            cv2.putText(top_array, str(int(obj[0])), (center[0] - 6,
                        center[1] + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, 0, 1)
...
def print_report(detections, depth_frame, rgb_ints, class_names):
    ...
    if not np.shape(detections)[0] == 0:
        ...
        for dets in detections:
            ...
            if class_names:
                rospy.loginfo(f"{class_names[int(dets[0])]} in x: {int(object_pose_cm[0])}, y: {int(object_pose_cm[1])}, d: {int(object_pose_cm[4])} with score: {dets[1]:.2f}")
            else:
                rospy.loginfo(f"Class ID: {int(dets[0])} in x: {int(object_pose_cm[0])}, y: {int(object_pose_cm[1])}, d: {int(object_pose_cm[4])} with score: {dets[1]:.2f}")
            
    else:
        rospy.loginfo("No object detected in the frame")
...
def capture_annotation_depth(detections, rgb_frame, depth_frame, class_names):
    ...
    for dets in detections:
        ...
        dep = depth_frame[int(dets[3] + dets[5]/2)
                                          ,int(dets[2] + dets[4]/2)]
        ...
        cv2.rectangle(annot_array, (int(dets[2]), int(dets[3]))
                      , (int(dets[2] + dets[4]), int(dets[3]) + 15), (0, 0, 255), -1)
        cv2.rectangle(annot_array, (int(dets[2]), int(dets[3] + dets[5]) - 15)
                      , (int(dets[2] + dets[4]), int(dets[3] + dets[5])), (0, 0, 255), -1)
        
        cv2.putText(
            annot_array, f"C: {dets[0]}",
            (int(dets[2]) + 3, int(dets[3]) + 15 - 3),
        ...
        cv2.putText(
            annot_array, f"D: {dep:.1f}",
            (int(dets[2]) + 3, int(dets[3] + dets[5]) - 3),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.2,
            (255, 255, 255),
            1,
